terraform {
  ...
  backend "local" {
    path = "../../../.private/tf-states/vcd-ubuntucluster.tfstate"
  }
}
...
resource "vcd_vapp" "ubuntucluster" {
  name        = "UbuntuCluster"
  description = "Ubuntu Cluster vApp"
  power_on    = true
}

resource "vcd_vapp_org_network" "ubuntucluster_wanInet" {
  vapp_name              = vcd_vapp.ubuntucluster.name
  org_network_name       = data.vcd_network_direct.wan_inet.name
  reboot_vapp_on_removal = true
}

resource "vcd_vapp_org_network" "ubuntucluster_lanMgmt" {
  vapp_name              = vcd_vapp.ubuntucluster.name
...
module "vms_ubuntucluster" {
  ...
  networks = [
    {
      name       = vcd_vapp_org_network.ubuntucluster_wanInet.org_network_name
      is_primary = true
    },
    {
      name       = vcd_vapp_org_network.ubuntucluster_lanMgmt.org_network_name
      is_primary = false
    }
  ]
...
module "data_state_ubuntucluster_vms" {
  source = "../../modules/save-data-state"
  init   = module.init
  id     = "ubuntucluster"
  data = {
    instances = [
      for k, bd in module.vms_ubuntucluster : bd.data
    ],
  }
}
